				...
				<div class="row">
					<div class="col-md-10 col-md-offset-4">
						...
						<?php
							...
							$db=new DB_Connect();
							$conn=$db->connect();
							$sql="select tentap,linkphim from tapphim where id='$id'";
							...
							if(isset($_POST["submit"]))
							{
								...
								if($result)
								{
									if(mysqli_num_rows($result)>0)
									{
										...
										$sql="Update tapphim set tentap='$tentap',linkphim='$linkphim' where id='$id'";
										$result=$conn->query($sql);
										if($result)
										{
											$result=$conn->query("select phim from tapphim where id='$id'");
											$id_phim=mysqli_fetch_row($result)[0];
											header("Location: /phimhay/admin/tapphim.php?id=$id_phim");
										}else{
											echo '<script language="javascript">';
							...
							<form action="#" role="form" method="post">
								<table>
									<tr>
										<td><label>Tên tập</label></td>
										<td><input type="text" name="tentap" class="form-control" value="<?php echo $row['0'] ?>"></td>
									</tr>
									<tr>
										<td><label>Link phim</label></td>
										<td><input type="text" name="linkphim" class="form-control" value="<?php echo $row['1'] ?>"></td>
									</tr>
									<tr>
										<td></td>
